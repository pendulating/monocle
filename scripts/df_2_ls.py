...
try:
    ...
    try:
        ...
        # Fallback to a simple logger if setup_logger is not found
        import logging
        def setup_logger(name):
            logging.basicConfig(level=logging.INFO)
            logger = logging.getLogger(name)
            if not hasattr(logger, 'success'):
                setattr(logger, 'success', lambda msg: logger.info(f"SUCCESS: {msg}"))
            return logger
...
class DF_2_LS:
    def __init__(self): 
        ...
        self.alternative_columns = ["img_path"]
    ...
    def convert(self, output_path):
        ...
        img_col = None
        for col in self.needed_columns + self.alternative_columns:
            if col in self.df.columns:
                img_col = col
                break
        
        if img_col is None:
            raise ValueError(f"df must have one of the following columns: {self.needed_columns + self.alternative_columns}")
        

        ls_data = []

        # map image_path to image with f'/data/local-files/?d={d["image"]}.jpg',
        self.df["image"] = self.df[img_col].apply(lambda x: f"/data/local-files/?d={x}")

        # columns to drop from the task metadata (to keep JSON small)
        cols_to_drop = [img_col, "image_base64", "image_url"]

        for idx, row in self.df.iterrows():

            # turn df columns into a dictionary
            # handle NaN values by converting them to None for valid JSON
            row_dict = row.where(pd.notnull(row), None).to_dict()

            # remove unnecessary columns from dictionary
            for col in cols_to_drop:
                if col in row_dict:
                    del row_dict[col]
            
            # Label Studio expects data to be nested under a "data" key
            task = {
                "data": row_dict,
                "id": idx
            }
