...
_PATCH_MODULE = "dagspaces.urbanvqa.resource_tracker_patch"
...
def _wrap_cleanup_funcs(rt_module) -> None:
    ...
    for rtype, func in list(rt_module._CLEANUP_FUNCS.items()):  # noqa: SLF001
        ...
        if getattr(func, "__urbanvqa_wrapped__", False):
            continue
        ...
        _wrapped.__urbanvqa_wrapped__ = True  # type: ignore[attr-defined]
            ...
            for raw_line in pipe:
                ...
                try:
                    ...
                    if cmd == "REGISTER":
                        ...
                        # Discard instead of remove to avoid noisy KeyError traces.
                        cache.setdefault(rtype, set()).discard(name)
                    elif cmd == "PROBE":
                        continue
                    else:
                        raise RuntimeError(f"unrecognized command {cmd!r}")
                except Exception:  # pragma: no cover - mirrors stdlib behaviour
                    exc_type, exc, tb = sys.exc_info()
                    # Swallow benign KeyError that may still slip through from
...
def apply_patch() -> None:
    ...
    # Monkey-patch the class and module-level helpers.
