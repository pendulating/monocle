...
python -m dagspaces.urbanvqa.run_pipeline \
...
# See dagspaces/urbanvqa/conf/pipeline/event_detection_from_articles.yaml for example
```
...
1. **Copy a base config**:
   ```bash
   cp dagspaces/urbanvqa/conf/topic/event_detection.yaml \
      dagspaces/urbanvqa/conf/topic/my_custom.yaml
   ```
...
1. Create `dagspaces/urbanvqa/conf/topic/your_config.yaml`
