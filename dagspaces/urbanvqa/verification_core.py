...
def init_verification(
    ...
    # Resolve debug flag from argument or env var URBANVQA_VERIFY_DEBUG
    try:
        _DEBUG = bool(debug) if debug is not None else (str(os.environ.get("URBANVQA_VERIFY_DEBUG", "")).strip().lower() in {"1","true","yes","on"})
    except Exception:
        _DEBUG = False
    _CONFIG = VerificationConfig(
        method=str(method),
    ...
    if _EMBED_MODEL is None:
        try:
            ...
            if _DEBUG:
                try:
                    print(f"[URBANVQA][verify] Loaded embedding model: {_CONFIG.embed_model_name} on {_DEVICE}", flush=True)
                except Exception:
                    pass
            ...
            if _DEBUG:
                print(f"[URBANVQA][verify] Failed to load embedding model '{_CONFIG.embed_model_name}': {e}", flush=True)
    ...
    if _CONFIG.method in {"nli", "combo", "combo_judge"} and _NLI_MODEL is None:
        try:
            ...
            if _DEBUG:
                try:
                    print(f"[URBANVQA][verify] Loaded NLI model: {_CONFIG.nli_model_name} on {_DEVICE}", flush=True)
                except Exception:
                    pass
        except Exception as e:
            ...
            if _DEBUG:
                print(f"[URBANVQA][verify] Failed to load NLI model '{_CONFIG.nli_model_name}': {e}", flush=True)
