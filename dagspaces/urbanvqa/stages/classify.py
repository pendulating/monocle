...
def _load_image_from_path(path: str) -> Optional[str]:
    """Load image from local path or URL and convert to base64 string.
    
    CRITICAL: Returns base64 string (PyArrow-serializable), NOT PIL Image.
    PIL Images are never needed - vLLM accepts base64 strings directly.
    ...
    Returns:
        Base64 string with data URI prefix (e.g., "data:image/jpeg;base64,..."), or None on failure
    """
    import base64
    from io import BytesIO
    ...
    try:
        ...
        if path.startswith(("http://", "https://")):
            if not _REQUESTS_AVAILABLE:
                ...
                return None
            response = requests.get(path, timeout=10, headers={"User-Agent": "URBANVQA/1.0"})
            response.raise_for_status()
            pil_image = PIL.Image.open(BytesIO(response.content)).convert("RGB")
...
def _load_image_from_base64(base64_str: str) -> Optional[str]:
    """Load image from base64-encoded string and return as base64 string.
    
    CRITICAL: Returns base64 string (PyArrow-serializable), NOT PIL Image.
    If input is already a valid base64 string, returns it as-is.
    ...
    try:
        ...
        print(f"Warning: Failed to process base64 image: {e}", flush=True)
        return None


def _normalize_image(image: Any) -> Optional[str]:
    """Normalize various image formats to base64 string.
    
    CRITICAL: Returns base64 string (PyArrow-serializable), NOT PIL Image.
    PIL Images are never needed - vLLM accepts base64 strings directly.
    
    Supports:
    - PIL.Image.Image: Convert to base64
    - numpy.ndarray: Convert array to base64
    - bytes: Load from bytes buffer and convert to base64
    - str: Try base64 decode (if already base64), or path/URL load and convert to base64
    
    Args:
        image: Image in various formats (numpy array, PIL Image, bytes, str)
    ...
    if isinstance(image, str):
        if image.startswith("data:image/"):
            return image
        # If it's a long string that looks like base64, add prefix
        if len(image) > 100:
    ...
    if _PIL_AVAILABLE and (hasattr(image, "convert") or hasattr(image, "size")):
        try:
            buffer = BytesIO()
            ...
            print(f"Warning: Failed to convert PIL Image to base64: {e}", flush=True)
            return None
    
    # Convert numpy array to base64
    if np is not None and isinstance(image, np.ndarray):
        try:
            # Ensure array is uint8 and in correct format
            img_val = image.copy()
            if img_val.dtype != np.uint8:
                if img_val.max() <= 1.0:
                    img_val = (img_val * 255).astype(np.uint8)
                else:
                    img_val = img_val.astype(np.uint8)
            
            # Handle different channel orders (HWC vs CHW)
            if len(img_val.shape) == 3 and img_val.shape[0] < img_val.shape[2]:
                # Likely CHW format, transpose to HWC
                img_val = img_val.transpose(1, 2, 0)
            
            # Convert to PIL Image temporarily for encoding
            if _PIL_AVAILABLE:
                pil_img = PIL.Image.fromarray(img_val).convert("RGB")
                buffer = BytesIO()
                pil_img.save(buffer, format="JPEG")
                base64_bytes = base64.b64encode(buffer.getvalue())
                return f"data:image/jpeg;base64,{base64_bytes.decode('utf-8')}"
        except Exception as e:
            print(f"Warning: Failed to convert numpy array to base64: {e}", flush=True)
            return None
    ...
    if isinstance(image, bytes):
        try:
            if _PIL_AVAILABLE:
                pil_image = PIL.Image.open(BytesIO(image)).convert("RGB")
                buffer = BytesIO()
                pil_image.save(buffer, format="JPEG")
                base64_bytes = base64.b64encode(buffer.getvalue())
                return f"data:image/jpeg;base64,{base64_bytes.decode('utf-8')}"
        except Exception as e:
            print(f"Warning: Failed to convert bytes to base64: {e}", flush=True)
            return None
    
    return None


def _load_image_from_row(row: Dict[str, Any]) -> Optional[str]:
    """Load image from various sources in row and convert to base64 string.
    ...
    5. image (if numpy array, converts to base64; if PIL Image, converts to base64)
    
    Returns base64 string with data URI prefix, or None on failure.
    
    Args:
        row: Dictionary containing image data in various formats
            - 'image': numpy.ndarray (from ray.data.read_images) or PIL.Image
    ...
    Returns:
        Base64 string with data URI prefix (e.g., "data:image/jpeg;base64,..."), or None if no image found or loading failed
    """
...
def _convert_numpy_to_pil_map(row: Dict[str, Any]) -> Dict[str, Any]:
    ...
    CRITICAL: This function now converts numpy arrays to base64 strings, NOT PIL Images.
    PIL Images are never needed - vLLM accepts base64 strings directly.
    ...
    Returns:
        Dictionary with 'image' column containing base64 string (or None)
    """
    if "image" in row:
        if isinstance(row["image"], np.ndarray):
            row["image"] = _normalize_image(row["image"])
        elif isinstance(row["image"], str):
            # Already a string (base64 or path) - normalize it
            row["image"] = _normalize_image(row["image"])
        elif _PIL_AVAILABLE and isinstance(row["image"], PIL.Image.Image):
...
def _load_images_map(row: Dict[str, Any]) -> Dict[str, Any]:
    ...
    CRITICAL: This function now converts images to base64 strings, NOT PIL Images.
    PIL Images are never needed - vLLM accepts base64 strings directly.
    
    This is called lazily during Ray Dataset execution, not at dataset creation.
    Handles multiple input formats and converts to base64 string.
...
def run_classification_stage(df: pd.DataFrame, cfg):
    ...
    - LLM: borrow prompt shape from experiments/prompts/base.yaml (relevance_v1) when available,
      else fall back to URBANVQA classify prompt. Produces both `relevance_answer` and `is_relevant`.
    ...
    def _extract_matched_keywords(text: Any) -> tuple[bool, list[str], int]:
        ...
        try:
            ...
            return True, [], 0
    # Prefer experiments-style prompts when available; otherwise fall back to URBANVQA prompt
    if is_eu_profile:
    ...
    def _format_user(article_text: str, row: Dict[str, Any]) -> str:
        ...
        if not art_id:
            try:
                ...
                art_id = "unknown"
        # If template uses '{chunk_text}' style, format with .format; else support URBANVQA '{{rule_text}}'
        if "{chunk_text}" in user_template or "{article_id}" in user_template:
            try:
                ...
                pass
        # Maintain URBANVQA template compatibility
    ...
    if "tensor_parallel_size" not in ek and tp_env:
        try:
            ...
            if not os.environ.get("RULE_TUPLES_SILENT"):
                print(f"Using tensor_parallel_size={tp_val} from URBANVQA_TENSOR_PARALLEL_SIZE")
        except Exception:
            pass
    ...
    def _pre(row: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if is_multimodal and image is not None:
            # Multimodal format: content is a list with text and image (base64 string)
            # image is now a base64 string, not a PIL Image
            user_content = [
                {"type": "text", "text": user},
    ...
    if is_ray_ds:
        ...
        if prefilter_mode in ("pre_gating", "post_gating"):
            ...
            try:
                if _RAY_OK:
                    usage_actor = ray.get_actor("urbanvqa_usage_agg")  # type: ignore
            except Exception:
                usage_actor = None  # type: ignore
