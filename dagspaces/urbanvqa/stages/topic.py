...
def _get_logger() -> logging.Logger:
    logger = logging.getLogger("urbanvqa.topic")
    if not logger.handlers:
        level_name = str(os.environ.get("URBANVQA_TOPIC_LOG_LEVEL", "INFO")).upper()
        try:
            logger.setLevel(getattr(logging, level_name, logging.INFO))
        except Exception:
            logger.setLevel(logging.INFO)
        handler = logging.StreamHandler(sys.stdout)
        handler.setFormatter(logging.Formatter("%(asctime)s %(levelname)s urbanvqa.topic - %(message)s"))
...
def _log_event(event: str, logger=None, **kwargs: Any) -> None:
    ...
    if logger:
        try:
            ...
            gate = str(os.environ.get("URBANVQA_TOPIC_WANDB_LOG_EVENTS", "1")).strip().lower() in ("1","true","yes","on")
            if gate:
...
def run_topic_stage(df, cfg, logger=None):
    ...
    try:
        ...
        eff_dev_pre = sel_dev
        try:
            disable_auto = str(os.environ.get("URBANVQA_TOPIC_DISABLE_AUTO_PROMOTE", "")).strip().lower() in ("1","true","yes","on")
    ...
    try:
        show_pb = str(os.environ.get("URBANVQA_TOPIC_SHOW_PB", "")).strip().lower() in ("1","true","yes","on")
    except Exception:
        show_pb = False
    ...
    try:
        if show_pb:
            ...
            try:
                log_every_env = os.environ.get("URBANVQA_TOPIC_LOG_EVERY", "")
