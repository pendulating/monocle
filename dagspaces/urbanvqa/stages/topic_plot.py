...
def log_cluster_scatter_to_wandb(df, logger=None, title: str = "topic_cluster_map") -> Optional[object]:
    ...
    try:
        max_rows_env = os.environ.get("URBANVQA_TOPIC_TABLE_MAX_ROWS")
    ...
    try:
        if len(df) > table_limit:
            ...
            try:
                seed_env = os.environ.get("URBANVQA_WB_TABLE_SEED") or os.environ.get("URBANVQA_TABLE_SAMPLE_SEED")
...
def log_cluster_scatter_plotly_to_wandb(df, logger=None, title: str = "topic_cluster_map") -> Optional[object]:
    ...
    seed = 777
    max_points = _get_env_int("URBANVQA_TOPIC_PLOT_MAX_POINTS", 120000)
    heat_threshold = _get_env_int("URBANVQA_TOPIC_PLOT_HEATMAP_THRESHOLD", 250000)
    heat_bins = _get_env_int("URBANVQA_TOPIC_HEATMAP_BINS", 200)
    marker_size = _get_env_int("URBANVQA_TOPIC_PLOT_MARKER_SIZE", 3)
    try:
        op_env = os.environ.get("URBANVQA_TOPIC_PLOT_MARKER_OPACITY")
        marker_opacity = float(op_env) if op_env is not None else 0.6
    except Exception:
        marker_opacity = 0.6
    method = os.environ.get("URBANVQA_TOPIC_PLOT_METHOD", "auto").strip().lower()

    # Default large-plot optimizations: enable when n > 10k
    optimize_threshold = _get_env_int("URBANVQA_TOPIC_PLOT_OPTIMIZE_THRESHOLD", 10000)
    optimize_cap = _get_env_int("URBANVQA_TOPIC_PLOT_MAX_POINTS_LARGE", 10000)
