...
Functions and classes extracted from urbanvqa, urbanocr, and other dagspace
orchestrators. Each orchestrator imports from this module and re-exports
the symbols for backward-compatibility with runner files that do
    ...
    *,
    ...
    try:
        ...
        if model_cfg:
            ...
            if model_source:
                run_config["model_source"] = str(model_source)
    except Exception:
        pass
...
def _load_parquet_dataset(
    ...
    # Apply sample_n regardless of debug flag
    if isinstance(sample_n, int) and sample_n > 0:
        try:
            ...
            n = int(sample_n)
        try:
            seed_env = os.environ.get("MLLMSCI_SAMPLE_SEED", "777")
...
def prepare_stage_input(
    ...
    Returns ``(df, None, False)`` -- the tuple shape is kept for backward
    compatibility with existing runner code.
    """
...
def _adjust_tensor_parallel_env(
    valid_count: int, env_prefix: str = "MLLMSCI"
) -> None:
...
def _log_gpu_environment(reason: str, env_prefix: str = "MLLMSCI") -> None:
...
def _sanitize_cuda_visible_devices(
    ...
    # Clamp tensor-parallel in cfg when provided
    if cfg is not None:
        try:
