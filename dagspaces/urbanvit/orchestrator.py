"""UrbanViT pipeline orchestrator.

Imports shared infrastructure from dagspaces.common and adds urbanvit-specific
stage runners for ViT training, feature extraction, and classification.
...
from .stages.shard import run_shard_stage
from .stages.train import run_train_stage
from .stages.eval import run_eval_stage
from .stages.extract import run_extract_stage
from .stages.classify import run_classify_stage
from .stages.collate import run_collate_stage

_CONFIG_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), "conf")
_DAGSPACE_NAME = "urbanvit"
_JOB_PREFIX = "URBANVIT"


# ---------------------------------------------------------------------------
# Stage runners — thin wrappers that wire inputs and call stage functions
# ---------------------------------------------------------------------------

class ShardRunner(StageRunner):
    stage_name = "shard"

    def run(self, context: StageExecutionContext) -> StageResult:
        cfg = context.cfg

        shards_dir = context.output_paths.get("shards_dir")
        if shards_dir:
            OmegaConf.update(cfg, "shard.output_dir", shards_dir, merge=True)

        result = run_shard_stage(cfg)

        outputs = {"shards_dir": result["shards_dir"]}
        if "manifest" in result:
            outputs["manifest"] = result["manifest"]

        if context.logger:
            try:
                context.logger.log_grouped_metrics("shard", result.get("metrics", {}))
            except Exception:
                pass

        return StageResult(outputs=outputs, metadata=result.get("metadata", {}))


class TrainRunner(StageRunner):
    stage_name = "train"

    def run(self, context: StageExecutionContext) -> StageResult:
        cfg = context.cfg

        shards_dir = context.inputs.get("shards_dir")
        if shards_dir:
            OmegaConf.update(cfg, "shard.output_dir", shards_dir, merge=True)

        checkpoint_dir = context.output_paths.get("checkpoint")
        if checkpoint_dir:
            OmegaConf.update(cfg, "training.checkpoint_dir", checkpoint_dir, merge=True)

        result = run_train_stage(cfg)

        outputs = {"checkpoint": result["checkpoint_dir"]}
        if context.logger:
            try:
                context.logger.log_grouped_metrics("train", result.get("metrics", {}))
            except Exception:
                pass
        return StageResult(outputs=outputs, metadata=result.get("metadata", {}))


class EvalRunner(StageRunner):
    stage_name = "eval"
    ...
    def run(self, context: StageExecutionContext) -> StageResult:
        ...
        if shards_dir:
            OmegaConf.update(cfg, "shard.output_dir", shards_dir, merge=True)
        checkpoint = context.inputs.get("checkpoint")
        if checkpoint:
            OmegaConf.update(cfg, "training.checkpoint_dir", checkpoint, merge=True)

        metrics_path = context.output_paths.get("metrics")
        if metrics_path:
            OmegaConf.update(cfg, "eval.metrics_path", metrics_path, merge=True)

        result = run_eval_stage(cfg)

        outputs = {"metrics": result["metrics_path"]}
        if context.logger:
            try:
                context.logger.log_grouped_metrics("eval", result.get("metrics", {}))
            except Exception:
                pass
        return StageResult(outputs=outputs, metadata=result.get("metadata", {}))


class ExtractRunner(StageRunner):
    stage_name = "extract"
    ...
    def run(self, context: StageExecutionContext) -> StageResult:
        ...
        checkpoint = context.inputs.get("checkpoint")
        if checkpoint:
            OmegaConf.update(cfg, "extract.backbone_checkpoint", checkpoint, merge=True)

        features_dir = context.output_paths.get("features")
        if features_dir:
            OmegaConf.update(cfg, "extract.output_dir", features_dir, merge=True)

        result = run_extract_stage(cfg)

        outputs = {"features": result["features_dir"]}
        if context.logger:
            try:
                context.logger.log_grouped_metrics("extract", result.get("metrics", {}))
            except Exception:
                pass
        return StageResult(outputs=outputs, metadata=result.get("metadata", {}))


class ClassifyRunner(StageRunner):
    stage_name = "classify"

    def run(self, context: StageExecutionContext) -> StageResult:
        cfg = context.cfg

        features = context.inputs.get("features")
        if features:
            OmegaConf.update(cfg, "classify.features_input", features, merge=True)
        checkpoint = context.inputs.get("checkpoint")
        if checkpoint:
            OmegaConf.update(cfg, "training.checkpoint_dir", checkpoint, merge=True)

        scores_path = context.output_paths.get("scores")
        if scores_path:
            OmegaConf.update(cfg, "classify.output_path", scores_path, merge=True)

        result = run_classify_stage(cfg)

        outputs = {"scores": result["scores_path"]}
        if context.logger:
            try:
                context.logger.log_grouped_metrics("classify", result.get("metrics", {}))
            except Exception:
                pass
        return StageResult(outputs=outputs, metadata=result.get("metadata", {}))


class CollateRunner(StageRunner):
    stage_name = "collate"

    def run(self, context: StageExecutionContext) -> StageResult:
        cfg = context.cfg

        scores = context.inputs.get("scores")
        if scores:
            OmegaConf.update(cfg, "collate.scores_input", scores, merge=True)

        final_path = context.output_paths.get("final")
        if final_path:
            OmegaConf.update(cfg, "collate.output_path", final_path, merge=True)

        result = run_collate_stage(cfg)

        outputs = {"final": result["final_path"]}
        if context.logger:
            try:
                context.logger.log_grouped_metrics("collate", result.get("metrics", {}))
            except Exception:
                pass
        return StageResult(outputs=outputs, metadata=result.get("metadata", {}))
...
def execute_stage_job(context_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    with WandbLogger(cfg, stage=node.stage, run_id=wandb_run_id,
                     run_config=run_config) as logger:
        try:
...
def run_experiment(cfg: DictConfig) -> None:
    """Execute the configured urbanvit pipeline."""
...
def _submit_and_wait(
    ...
    job_name = f"{_JOB_PREFIX}-{node.key}"
