"""Artifact generation pipeline orchestrator.

Imports shared infrastructure from dagspaces.common and adds
artifact_gen-specific stage runners for geospatial raster generation.

Follows the same two-tier SLURM pattern as other dagspaces:
...
from .stages.raster import run_raster_stage
from .stages.combine import run_combine_stage
...
class RasterRunner(StageRunner):
    stage_name = "raster"

    def run(self, context: StageExecutionContext) -> StageResult:
        cfg = context.cfg

        # Wire inputs from pipeline sources
        embeddings_input = context.inputs.get("embeddings")
        if embeddings_input:
            OmegaConf.update(cfg, "raster.embeddings_input_path", embeddings_input, merge=True)

        pca_input = context.inputs.get("pca_artifacts")
        if pca_input:
            OmegaConf.update(cfg, "raster.pca_artifacts_path", pca_input, merge=True)

        proj_input = context.inputs.get("query_projection")
        if proj_input:
            OmegaConf.update(cfg, "raster.projection_matrix_path", proj_input, merge=True)

        results_path = context.output_paths.get("results")
        if results_path:
            OmegaConf.update(cfg, "runtime.output_path", results_path, merge=True)

        output_path = run_raster_stage(cfg)

        metadata = {
            "output_path": output_path,
            "query_text": str(cfg.raster.query_text),
            "resolution_m": float(cfg.raster.resolution_m),
            "interpolation": "idw",
        }

        if context.logger:
            try:
                context.logger.log_grouped_metrics("raster", metadata)
            except Exception:
                pass
...
class CombineRunner(StageRunner):
    stage_name = "combine"

    def run(self, context: StageExecutionContext) -> StageResult:
        cfg = context.cfg

        # Wire inputs from upstream raster nodes
        raster_a_input = context.inputs.get("raster_a")
        if raster_a_input:
            OmegaConf.update(cfg, "combine.raster_a_path", raster_a_input, merge=True)

        raster_b_input = context.inputs.get("raster_b")
        if raster_b_input:
            OmegaConf.update(cfg, "combine.raster_b_path", raster_b_input, merge=True)

        results_path = context.output_paths.get("results")
        if results_path:
            OmegaConf.update(cfg, "runtime.output_path", results_path, merge=True)

        output_path = run_combine_stage(cfg)

        metadata = {
            "output_path": output_path,
            "label_a": str(cfg.combine.label_a),
            "label_b": str(cfg.combine.label_b),
            "operations": list(cfg.combine.operations),
        }

        if context.logger:
            try:
                context.logger.log_grouped_metrics("combine", metadata)
            except Exception:
                pass
...
def run_experiment(cfg: DictConfig) -> None:
    """Execute artifact generation pipeline."""
...
def _submit_and_wait(
    ...
    job_name = f"ARTIFACT_GEN-{node.key}"
