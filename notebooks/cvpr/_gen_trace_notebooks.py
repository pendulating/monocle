"""Generate one trace notebook for each prompt.

Placeholders are @@CASE@@ and @@TITLE@@, not str.format braces: the notebook
body is full of f-strings, and doubling every brace makes it unreadable.
"""
from pathlib import Path

# case name -> (title, prompt folder). The folder is NOT always the case name:
# the recipe directories were named `road` and `subway` before the cases were
# named `road_quality` and `subway_safety`.
# All 7 cases of the battery, in the order the paper uses. The trace runs of
# 2026-08-14 cover every case, thus every case gets a notebook.
#
# Note on the 2 cases without a proxy folder of their own:
#   parks_plazas       -> `parks/`. The traces are 1 corpus, because 1 run
#                         judged parks and plazas together. Only the PROXY
#                         splits in two, thus `plazas/` holds no trace notebook.
#   street_photography -> its own folder, which holds figures only: the case
#                         has no outside measurement, so it has no recipe.
CASES = {
    "subway_safety": ("Subway safety", "subway"),
    "libraries": ("Libraries", "libraries"),
    "schools": ("Schools", "schools"),
    "road_quality": ("Road quality", "road"),
    "parks_plazas": ("Parks and plazas", "parks"),
    "restaurants": ("Restaurants", "restaurants"),
    "street_photography": ("Street photography", "street_photography"),
}
...
def _(mo):
    ...
    reads the **@@CASE@@** thinking runs and draws the words the model writes
    while it decides.
    ...
    Thus this notebook counts **every** case and draws only **@@CASE@@**: the
...
def _(mo):
    ...
    mo.md(
        f"""
    ### 4.3 What @@CASE@@ names that the other cases do not
...
out = Path("/share/pierson/matt/mllmsci/notebooks/cvpr")
for case, (title, folder) in CASES.items():
    body = TEMPLATE.replace("@@CASE@@", case).replace("@@TITLE@@", title)
    # Every artifact of a prompt lives inside that prompt's folder.
    d = out / folder
    d.mkdir(parents=True, exist_ok=True)
    p = d / f"{case}_traces.py"
    p.write_text(body)
